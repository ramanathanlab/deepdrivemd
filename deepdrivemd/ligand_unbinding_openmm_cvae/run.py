...
def run_simulation(
    ...
    from deepdrivemd.ligand_unbinding_openmm_cvae.simulation.app import (
        MDSimulationApplication,
    )
...
def run_train(input_data: CVAETrainInput, config: CVAETrainSettings) -> CVAETrainOutput:
    from deepdrivemd.ligand_unbinding_openmm_cvae.train.app import CVAETrainApplication
...
def run_inference(
    ...
    from deepdrivemd.ligand_unbinding_openmm_cvae.inference.app import (
        CVAEInferenceApplication,
    )
...
class DeepDriveMD_OpenMM_CVAE(DeepDriveMDWorkflow):
    def __init__(
        ...
        self.train_input = CVAETrainInput(contact_map_paths=[], energy_paths=[])
        ...
        self.inference_input = CVAEInferenceInput(
            contact_map_paths=[], energy_paths=[], model_weight_path=Path()
        )
    ...
    def handle_simulation_output(self, output: MDSimulationOutput) -> None:
        ...
        self.train_input.append(output.contact_map_path, output.energy_path)
        self.inference_input.append(output.contact_map_path, output.energy_path)
